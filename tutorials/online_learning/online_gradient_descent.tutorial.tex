    ...
    \title{\textbf{\textsf{\texttt{PEPit.jl} tutorial: Worst-case regret of online gradient descent}}}
...
The following code computes a worst-case guarantee for \textbf{online gradient descent} (OGD) with a step-size $\gamma=\frac{D}{M \sqrt{n}}$. That is, it computes the smallest possible $\tau(n, M, D)$ such that the guarantee
...
$\tau(n, M, D)$ is computed as the worst-case value of $R_n$. Let us see how to do it step by step in \texttt{PEPit.jl}!
...
Let us start with loading the necessary \texttt{PEPit.jl} related packages.
