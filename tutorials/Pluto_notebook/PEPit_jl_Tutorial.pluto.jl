...
Here we see an example of how to use `PEPit.jl` for analysis of accelerated gradient method for smooth strongly convex minimization.

## Setup
Before we do anything let us load the necessary packages.
"""

# ╔═╡ 75446fa0-2430-4976-92a3-933196fda44c
md"""
## Imports
...
# ╔═╡ 5512bbcb-98f4-4860-b4c1-c4d790c263da
md"""
Let us start with defining an empty PEP, which we will construct step by step.
"""

# ╔═╡ c87f745e-1dfd-425b-ae42-7c4f7da665e5
problem = PEP()

# ╔═╡ 4372d048-9468-42fc-84e5-7fc16a606567
md"""
...
# ╔═╡ b72f654f-17b8-4ecc-9649-87c36eb8642e
μ = 0.1

# ╔═╡ 14a995ef-877b-4888-9401-7fa0bf6b3dee
L = 1

# ╔═╡ b7db83a1-18a8-4c15-8642-e18b400b8fa0
param = OrderedDict("mu" => μ, "L" => L)

# ╔═╡ 860d7a9e-62c2-4430-aebb-7f29d33c2b4e
md"""
This defines the set of parameters $\mu$ and $L$ to describe the function class. Now we define the function class itself.  
"""

# ╔═╡ 1ebea84f-e96b-4676-a16f-08e9ce005064
...
# ╔═╡ 4cce6813-550b-4bc8-b676-d9673b97f00d
md"""
## Algorithm
...
# ╔═╡ 85a076e9-2f49-4c4f-b110-e8b4ab6dfc35
md"""
## Goal
...
is valid, where $x_n$ is the output of the **fast gradient** method, and where $x_\star$ is the minimizer of $f$. 

Sometimes we say that 

$$\left(f(x_0) -  f(x_\star) + \frac{\mu}{2}\|x_0 - x_\star\|^2\right)$$

is the initial condition for this PEP. 
...
# ╔═╡ 489d1419-3682-4c72-acd8-c436955bb46d
md"""
...
# ╔═╡ ee6e07b6-cc58-4f16-9f0d-bb88988e96e8
n = 2

# ╔═╡ e621e422-d8f4-455c-8682-0463c574c56a
md"""
...
# ╔═╡ aa1a3b15-f4cf-44fb-98cd-f00038218ac1
xs = stationary_point!(func)

# ╔═╡ 159963fc-97ea-4882-9472-fe3cc53d4d8a
fs = value!(func, xs) 

# ╔═╡ 31b94d08-a5a4-4f56-8e58-779d51a8e7b7
x0 = set_initial_point!(problem) 

# ╔═╡ 7166609e-6787-46d4-93c0-6808a905e406
md"""
...
# ╔═╡ 31a59aab-eb4a-456c-887c-6e306a75943f
set_initial_condition!(problem, value!(func, x0) - fs + μ / 2 * (x0 - xs)^2 <= 1) 

# ╔═╡ 745311eb-2dd9-40ea-960b-ebe86343aacd
md"""
...
# ╔═╡ 48e7d20f-6bf9-42e3-bb3b-d5fcf84a5aa9
κ = μ/L

# ╔═╡ 4230fcc1-7e64-4405-ba47-56cb099559af
begin
	x_new, y = x0, x0
	for i in 1:n
...
# ╔═╡ ce0600de-6d16-4313-a6f2-4d9809cebc65
md"""
So when the loop ends `x_new` contains $x_n$. Let us now evaluate $f(x_n)$.
"""

# ╔═╡ f33f6ad2-e2d6-488a-903b-743603dbb826
f_n = value!(func, x_new) 

# ╔═╡ 1ddd8369-b57c-4eb9-a70e-9a784f784549
md"""
Let us now tell the PEP that our performance measure is $f(x_N) - f_\star$.
"""

# ╔═╡ 3726f6e9-c3d9-45e1-9532-93d52b0e215a
set_performance_metric!(problem, value!(func, x_new) - fs)

# ╔═╡ e9e8e0de-91b2-4661-9a5e-da95a597f82d
md"""
### Solve the PEP
We have everything to build up the PEP. Now time to solve!
"""

# ╔═╡ e8907515-1afb-4c64-b5ba-c0d56c6c854a
τ_PEPit = solve!(problem; solver = Clarabel.Optimizer, verbose=true) 

# ╔═╡ 729e0db5-fb52-4fd0-aabb-c5192fc232de
md"""
### Comparison with theoretical results

If everything went well, we should get $(τ_PEPit) or something very close to it. What does the theory say in this regard? The following **upper** guarantee can be found in [7, Corollary 4.15]:
...
# ╔═╡ 5212925d-a333-4b38-82e8-87e6879d8565
τ_Theory = (1 - sqrt(κ))^n

# ╔═╡ 2fcec4ea-57e0-4257-beb3-5a91b2e4e448
@info "💻  PEPit guarantee: f(x_n)-f_*  <= $(round(τ_PEPit, digits=6)) (f(x_0) - f(x_*) + mu/2*||x_0 - x_*||^2)"

# ╔═╡ 38175b46-1655-42f8-afa9-27bf86541342
@info "📝 Theoretical guarantee: f(x_n)-f_*  <= $(round(τ_Theory, digits=6)) (f(x_0) - f(x_*) + mu/2*||x_0 - x_*||^2)"

# ╔═╡ d809b78f-65d7-4dc8-b1a6-927da8e75fb7
md"""
So the gurantee that we found via `PEPit` is tighter than the theoretical guarantee!
"""

# ╔═╡ 64488abc-08b6-47b8-9075-ce4be079cc95
md"""
## Putting everyting in a function. 

Let us put everyting that we did so far in a function. 
"""

# ╔═╡ b09d9423-6dda-4280-8bd9-53f429f27e38
function wc_accelerated_gradient_strongly_convex(μ, L, n; verbose=false)
    ...
    PEPit_τ = solve!(problem; solver = Clarabel.Optimizer, verbose=true) 
	
    theoretical_τ = (1 - sqrt(kappa))^n
	
    μ == 0 && @warn "Momentum is tuned for strongly convex functions!"

    if verbose
        @info "🐱 Example file: worst-case performance of the accelerated gradient method" 
        @info "💻  PEPit guarantee: f(x_n)-f_*  <= $(round(PEPit_τ, digits=6)) (f(x_0) - f(x_*) + μ/2*||x_0 - x_*||^2)"
        @info "📝 Theoretical guarantee: f(x_n)-f_*  <= $(round(theoretical_τ, digits=6)) (f(x_0) - f(x_*) + μ/2*||x_0 - x_*||^2)"
    end

    return PEPit_τ, theoretical_τ

end

# ╔═╡ a0c01316-f030-413e-8042-a3b84e99f36b
md"""
...
over $n \in \{n_\text{min},\dots,n_\text{max}\}$ and compute both:

1. the PEPit coefficient $\tau_N$ returned by `wc_gradient_descent`, and
2. the theoretical coefficient.
"""

# ╔═╡ 0f3e1b5d-607f-4113-93de-f5359d986f3c
function run_sweep_and_plot(; L=1.0, gamma=1.0, n_min=1, n_max=20, verbose=false)
    @assert n_min >= 1 "n_min must be at least 1."
    @assert n_max >= n_min "n_max must be greater than or equal to n_min."

    n_values = collect(n_min:n_max)
    pepit_values = Vector{Float64}(undef, length(n_values))
    theoretical_values = Vector{Float64}(undef, length(n_values))

	for (idx, n_current) in pairs(n_values)
		pepit_tau, theoretical_tau = wc_accelerated_gradient_strongly_convex(μ, L, n_current; verbose=false)
		pepit_values[idx] = pepit_tau
		theoretical_values[idx] = theoretical_tau
    end

	return n_values, pepit_values, theoretical_values
	
end

# ╔═╡ 7bb74608-4689-4404-82c0-fa133db8b993
md"""
Let us sweep over iteration bounds. 
"""

# ╔═╡ 06c4b057-b1d7-4f96-939e-d69e25e84b5f
n_values, pepit_values, theoretical_values = run_sweep_and_plot(; L=1.0, gamma=1.0, n_min=1, n_max=20, verbose=false)

# ╔═╡ 1bac4f76-4ad6-43fa-837a-df51f850d448
md"""
Time to plot!
"""

# ╔═╡ 966f25da-7e15-45ec-abc0-38a9a5e524e7
plot(n_values, [pepit_values, theoretical_values], title="", label=["PEPit.jl" "Theoretical Bound"], linewidth=3, xlabel= L"Iteration Number $n$", ylabel = L"Worst-case $f(x_n) - f_⋆$")

# ╔═╡ 2f67dbcc-8f62-4a90-a5f9-c0db32a389a9
md"""
## References 
...
[7] A. d’Aspremont, D. Scieur, A. Taylor (2021). Acceleration Methods. Foundations and Trends
...
manifest_format = "2.0"
project_hash = "194675b2c6b7e99c380a5e5b6b477a45ee6e2ed3"
...
git-tree-sha1 = "9b91261ed8d3d31a40839a558f4c3dd6113a15db"
...
[[deps.PEPit]]
deps = ["COPT", "Clarabel", "JuMP", "LinearAlgebra", "Literate", "Mosek", "MosekTools", "OffsetArrays", "OrderedCollections"]
git-tree-sha1 = "6ac2a32a05d7f25e28746ba2ea8b8e6bff9f19f8"
